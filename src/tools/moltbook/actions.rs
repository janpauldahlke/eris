...
use crate::tools::moltbook::client::{
    clean_path_segment, tool_result, validate_content_len, AuthMode, MoltbookClient,
};
...
impl Tool for MoltbookRegisterTool {
    fn name(&self) -> &'static str { "moltbook:register" }
    ...
    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(RegisterArgs) }
...
impl Tool for MoltbookStatusTool {
    fn name(&self) -> &'static str { "moltbook:status" }
    ...
    fn context_view_hint(&self) -> ToolContextViewHint {
        ToolContextViewHint::Snippet { max_chars: API_TOOL_SNIPPET_CHARS }
    }

    async fn execute(&self, args: Value) -> Result<String> {
        let _parsed = parse_args!(args, EmptyArgs);
        let status = self.client.get("/agents/status", &[], AuthMode::Bearer).await?;
        ...
        tool_result(self.name(), combined, "If status is pending_claim, ask the human to finish the claim flow.")
    }
}

#[async_trait]
impl Tool for MoltbookHomeTool {
    fn name(&self) -> &'static str { "moltbook:home" }
    ...
    fn context_view_hint(&self) -> ToolContextViewHint {
        ToolContextViewHint::Snippet { max_chars: API_TOOL_SNIPPET_CHARS * 2 }
    }
...
impl Tool for MoltbookFeedTool {
    fn name(&self) -> &'static str { "moltbook:feed" }

    fn description(&self) -> &'static str {
        "Read Moltbook feeds or post listings after an active user request."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(FeedArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        tool_result(self.name(), response, "Upvote or comment only when the user explicitly wants engagement or the content genuinely warrants it.")
    }
}

#[async_trait]
impl Tool for MoltbookCommentsTool {
    fn name(&self) -> &'static str { "moltbook:comments" }

    fn description(&self) -> &'static str {
        "Read comments on a Moltbook post."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(CommentsArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let response = self
            .client
            .get(&format!("/posts/{post_id}/comments"), &query, AuthMode::Bearer)
            .await?;
        tool_result(self.name(), response, "Reply only when the user asked or the conversation clearly needs a response.")
    }
}

#[async_trait]
impl Tool for MoltbookCommentTool {
    fn name(&self) -> &'static str { "moltbook:comment" }

    fn description(&self) -> &'static str {
        "Create a thoughtful Moltbook comment or reply. May return a verification challenge."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(CommentArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let response = self
            .client
            .post(&format!("/posts/{post_id}/comments"), Some(body), AuthMode::Bearer)
            .await?;
        tool_result(self.name(), response, "If verification_required is true, solve the challenge and call moltbook:verify before assuming the comment is visible.")
    }
}

#[async_trait]
impl Tool for MoltbookVoteTool {
    fn name(&self) -> &'static str { "moltbook:vote" }

    fn description(&self) -> &'static str {
        "Upvote or downvote Moltbook content. Use only for content Eris genuinely evaluated."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(VoteArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        tool_result(self.name(), response, "Summarize the vote. Follow suggestions only if the user asks.")
    }
}

#[async_trait]
impl Tool for MoltbookPostTool {
    fn name(&self) -> &'static str { "moltbook:post" }

    fn description(&self) -> &'static str {
        "Create a Moltbook text or link post. Use only when the user explicitly asks to post or approves a drafted post."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(PostArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        if let Some(kind) = parsed.kind {
            body["type"] = Value::String(match kind {
                PostKind::Text => "text",
                PostKind::Link => "link",
                PostKind::Image => "image",
            }.into());
        }
        let response = self.client.post("/posts", Some(body), AuthMode::Bearer).await?;
        tool_result(self.name(), response, "If verification_required is true, solve the challenge and call moltbook:verify before saying the post is published.")
    }
}

#[async_trait]
impl Tool for MoltbookVerifyTool {
    fn name(&self) -> &'static str { "moltbook:verify" }

    fn description(&self) -> &'static str {
        "Submit an answer to a Moltbook AI verification challenge."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(VerifyArgs) }

    async fn execute(&self, args: Value) -> Result<String> {
        let parsed = parse_args!(args, VerifyArgs);
        let verification_code = validate_content_len("verification_code", &parsed.verification_code, 8, 256)?;
        let answer = validate_content_len("answer", &parsed.answer, 1, 64)?;
        let response = self
            .client
            .post(
                "/verify",
            ...
            .await?;
        tool_result(self.name(), response, "Report whether the verification succeeded. Do not retry repeatedly after failures.")
    }
}

#[async_trait]
impl Tool for MoltbookNotificationsReadTool {
    fn name(&self) -> &'static str { "moltbook:notifications_read" }

    fn description(&self) -> &'static str {
        "Mark Moltbook notifications read after they have been handled."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(NotificationsReadArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let response = self.client.post(&path, None, AuthMode::Bearer).await?;
        tool_result(self.name(), response, "Confirm notifications were marked read.")
    }
}

#[async_trait]
impl Tool for MoltbookDmTool {
    fn name(&self) -> &'static str { "moltbook:dm" }

    fn description(&self) -> &'static str {
        "Check or manage Moltbook direct messages. New requests and human-input flags require operator involvement."
    }

    fn parameters_schema(&self) -> RootSchema { schemars::schema_for!(DmArgs) }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let response = match parsed.action {
            DmAction::Check => self.client.get("/agents/dm/check", &[], AuthMode::Bearer).await?,
            DmAction::ListRequests => {
                self.client.get("/agents/dm/requests", &[], AuthMode::Bearer).await?
            }
            DmAction::ListConversations => {
                self.client.get("/agents/dm/conversations", &[], AuthMode::Bearer).await?
            }
            DmAction::ReadConversation => {
                let id = required_segment(parsed.conversation_id.as_deref(), "conversation_id")?;
                self.client
                    .get(&format!("/agents/dm/conversations/{id}"), &[], AuthMode::Bearer)
                    .await?
            }
            DmAction::SendRequest => {
                ...
                match (parsed.to, parsed.to_owner) {
                    (Some(to), None) => body["to"] = Value::String(validate_content_len("to", &to, 1, 80)?),
                    (None, Some(to_owner)) => {
                        body["to_owner"] = Value::String(validate_content_len("to_owner", &to_owner, 1, 80)?);
                    }
                    _ => {
                        return Err(FcpError::SchemaViolation(
                ...
                self.client.post("/agents/dm/request", Some(body), AuthMode::Bearer).await?
            }
...
mod tests {
    ...
    async fn home_sends_auth_and_returns_rate_limit() {
        ...
        let client = Arc::new(MoltbookClient::for_test(
    ...
    async fn dm_send_request_requires_one_recipient() {
        ...
        })).await.expect_err("schema violation");
