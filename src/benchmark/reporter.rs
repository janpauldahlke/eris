...
impl ReportGenerator {
    ...
    pub fn console(report: &BenchmarkReport) -> String {
        ...
        output.push_str(&format!(
            ...
            checkbox(
                report.cleanup_report.staged_memories_removed > 0
                    || report.cleanup_report.all_cleaned()
            ),
        ...
        output.push_str(&format!(
            ...
            checkbox(
                report.cleanup_report.ephemeral_entries_removed > 0
                    || report.cleanup_report.all_cleaned()
            ),
        ...
        if !report.cleanup_report.cleanup_failures.is_empty() {
            output.push_str(&format!(
                "\n  ⚠ {} cleanup failures",
                report.cleanup_report.cleanup_failures.len()
            ));
        }
        ...
        if report.suite_speed.step_samples > 0 {
            output.push_str(&format!(
                "\n  {:.<40} {:>8} steps",
                "Samples (user steps)", report.suite_speed.step_samples
            ));
            output.push_str(&format!(
                "\n  {:.<40} {:>8} scenarios",
                "Contributing scenarios", report.suite_speed.contributing_scenarios
            ));
            output.push_str(&format!(
                "\n  {:.<40} {:>8.0} ms",
                "Mean LLM ms / step", report.suite_speed.mean_llm_ms
            ));
            output.push_str(&format!(
                "\n  {:.<40} {:>8.0} ms",
                "Mean tool ms / step", report.suite_speed.mean_tool_ms
            ));
            output.push_str(&format!(
                "\n  {:.<40} {:>8.0} ms",
                "Mean total ms / step", report.suite_speed.mean_total_ms
            ));
        } else {
    ...
    pub fn markdown(report: &BenchmarkReport) -> String {
        ...
        md.push_str(&format!(
            "| Timeout Rate | {:.1}% | {} |\n",
            report.quality.timeout_rate(),
            if report.quality.timeout_rate() < 5.0 {
                "✓ Good"
            } else {
                "⚠ High"
            }
        ...
        if !report.quality.failure_analyses.is_empty() {
            ...
            for (i, failure) in report.quality.failure_analyses.iter().enumerate() {
                ...
                md.push_str("  ```json\n");
                let preview = failure.raw_llm_output.chars().take(500).collect::<String>();
                md.push_str(&format!("  {}\n", preview));
    ...
    pub fn comparison(baseline: &BenchmarkReport, current: &BenchmarkReport) -> String {
        let mut output = String::new();
        ...
        output.push_str(&format!(
            "\n  Baseline:  {} ({})",
            baseline.model_name, baseline.run_id
        ));
        output.push_str(&format!(
            "\n  Current:   {} ({})",
            current.model_name, current.run_id
        ));
        output.push_str(&format!("\n{}", "─".repeat(70)));
        ...
        output.push_str(&format!(
            ...
            current.speed.total_duration.as_millis() as i128
                - baseline.speed.total_duration.as_millis() as i128
        ));
        output.push_str(&format!(
            "\n  {:.<35} {:>8.0}  {:>8.0}  {:>+6.0}",
            "Prompt phase (ms)",
            ...
            current.speed.prompt_eval_duration.as_millis() as i128
                - baseline.speed.prompt_eval_duration.as_millis() as i128
        ));
        output.push_str(&format!(
            "\n  {:.<35} {:>8.0}  {:>8.0}  {:>+6.0}",
            "Generation phase (ms)",
            ...
            current.speed.eval_duration.as_millis() as i128
                - baseline.speed.eval_duration.as_millis() as i128
        ));
        ...
        output.push_str(&format!(
            "\n  {:^66}",
            "SUITE TIMING (passed scenarios only)"
        ));
        output.push_str(&format!("\n{}", "─".repeat(70)));
        output.push_str(&format!(
            "\n  {:.<35} {:>8.0}  {:>8.0}  {:>+6.0}",
    ...
    pub fn generate_trend_report(reports: &[BenchmarkReport]) -> String {
        ...
        for report in reports {
            output.push_str(&format!(
                ...
                format!(
                    "{} {}",
                    report.timestamp.format("%m-%d"),
                    report.model_name.chars().take(15).collect::<String>()
                ),
                report.quality.json_success_rate(),
                report.quality.recovery_success_rate(),
                report.quality.tool_valid_rate(),
