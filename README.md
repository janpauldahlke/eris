...
- **In scope:** Interactive `chat` from a vault directory (`cd` into the vault, then `eris chat` or `eris chat --web`), tool use (vault, memory, web, agenda, clocks, German public-transit connections via `db:find_connections`, Gmail when Google Workspace is configured, etc.), structured logging under `.fcp/telemetry/logs/`. First-time **setup welder** (when `.fcp/seal` is missing and stdin is a TTY) probes Ollama/Qdrant/Docker/PATH and walks through vault confirmation before the ignition wizard writes config.
