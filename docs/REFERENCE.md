...
Direct inference via `llama-server`. Eris manages the server processes, compiles a **GBNF grammar** at session start that structurally enforces the JSON protocol at the token level, and constrains tool arguments per-tool. (Grammar guarantees output *shape*; completion is additionally protected by the bounded `n_predict_max` generation cap.)
...
- **CI:** GitHub Actions runs the same batch list on push (coverage/Codecov was removed for the open-source release; CI is clippy + sharded tests).
...
Debug tip — introspect the full llama.cpp HTTP traffic: `sudo tcpdump -i lo -s 0 -l -A 'tcp port 8090'`

## License

Copyright 2026 Jan Dahlke. Licensed under the [Apache License 2.0](../LICENSE). Contributions are accepted under the same license (inbound = outbound) with a [DCO](https://developercertificate.org/) sign-off; see [CONTRIBUTING.md](../CONTRIBUTING.md).
